...
void loop() {
    ...
    FinishCycle();  // while new power setting stabilizes: update energy stats, do maintenance tasks, read PV power, flash LED
    CheckErrors();  // check for comms errors, halt system if an error is persistent
    UserIO();  // read user command, print cycle info and user command response, check OTA software update request
}
...
bool ShellyCommand(IPAddress ip_addr, const char command[], const char params[]) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (!strcmp(command, MW_RELAY)) {
            ...
            delay(1000);  // add a little more time for output power to stabilize
            return true;
        }
        if (!strcmp(command, HM_RELAY)) {
            http.end();
            ts_HM = millis();
            // Hoymiles AC side turned on/ff: turn on/off DC side, too
    ...
    http.end();
    if (ip_addr == EM_ADDR) sprintf(error_msg, "3EM cmd %s failed", http_command);
    if (ip_addr == PM1_ADDR) sprintf(error_msg, "1PM cmd %s failed", http_command);
    if (ip_addr == PM2_ADDR) sprintf(error_msg, "2PM cmd %s failed", http_command);
    return false;
}
...
bool BMSCommand(const byte command[]) {
    ...
    if (command[0] == BLE_1) {
        ...
        ts_BMS = millis();
        sprintf(error_msg, "BMS BLE command 0x%02X 0x%02X failed", command[BLE_COMMAND_POS], command[BLE_SETTING_POS]);
        return false;
    }
    ...
    sprintf(error_msg, "BMS RS485 command 0x%02X 0x%02X failed", command[RS485_COMMAND_POS], command[DATA_ID_POS]);
    return false;
}
...
void FinishCycle() {
    ...
    if (mw_on && ((millis()-ts_MW)/1000 >= MW_KEEPALIVE)) ShellyCommand(PM2_ADDR, MW_RELAY, "on&timer=60");
    ...
    if (!power_new && !power_pv && !hm_awake && !pm2_eco_mode) pm2_eco_mode = ShellyCommand(PM2_ADDR, ECO_MODE, "true}}");
    ...
    if ((vcell_min >= ESS_UVPR-20) && !hm_awake) hm_awake = ShellyCommand(PM2_ADDR, HM_RELAY, "on");  // wake up HM before reaching ESS_UVPR, to allow time for AC sync procedure
    ...
    // daytime: read PV power from Shelly 1PM (normally takes less than 100 ms)
...
void CheckErrors() {
    ...
    HoymilesCommand(HM_POWER_OFF);
    ShellyCommand(PM2_ADDR, HM_RELAY, "off");
    SetMWPower(MW_MIN_POWER);
...
void UserIO() {
    ...
    // wait for power changes to stabilize
...
void PrepareCycleMsg() {
    ...
    strcat(cycle_msg, NIGHT_DAY_SYMBOL[(min_of_day >= sunrise) && (min_of_day < sunset)]);
