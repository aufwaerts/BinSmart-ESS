...
void setup() {
    ...
    if (!BMSCommand(BMS_SETTINGS)) telnet.println(ERROR_SYMBOL + error_msg);
    else telnet.println("RS485 communication with JKBMS OK");
    if (!BMSCommand(BMS_VOLTAGES)) telnet.println(ERROR_SYMBOL + error_msg);
    delay(1000);

    // Init BLE communication with BMS, turn off balancer switch
    NimBLEDevice::init("");
    if (!BLECommand(BLE_BAL_OFF)) telnet.println(ERROR_SYMBOL + error_msg);
...
void loop() {
    ...
    BMSCommand(BMS_VOLTAGES);  // Read cell voltages from BMS
    ...
    BMSCommand(BMS_CURRENT);  // Read DC charging/discharging current from BMS
...
bool BMSCommand(const byte command[]) {
    ...
    Serial2.write(command, BMS_COMMAND_LEN);
    Serial2.flush();
    ...
    error_msg = "BMS RS485 command failed";
    return false;
}

bool BLECommand(const byte command[]) {
    ...
    error_msg = "BMS BLE command failed";
    return false;
}
