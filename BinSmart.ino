...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        PrepareCycleMsg();
        ProcessUserCommand();
        telnet.printf("%s%s", cycle_msg, cmd_resp);
...
void PrepareCycleMsg() {
    ...
    if (errors_consecutive >= ERROR_LIMIT) {
        // System halted: prepare cycle message accordingly
...
void ProcessUserCommand() {
    ...
    if (errors_consecutive >= ERROR_LIMIT) {
        // System halted: prepare command response message accordingly
        command = '\0';
