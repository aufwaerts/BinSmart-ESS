...
void setup() {
    ...
    telnet.printf("%sBinSmart ESS %s\r\n\nWiFi connected to %s   RSSI: %d\r\n", CLEAR_SCREEN, SW_VERSION, WIFI_SSID, WiFi.RSSI());
    delay(2000);

    // Init PWM generator (for adjusting Meanwell power)
    if (!ledcAttachChannel(PWM_OUTPUT_PIN, PWM_FREQ, PWM_RESOLUTION, PWM_CHANNEL)) strcpy(error_msg, "PWM generator init failed");
    CheckErrors();
    ...
    // Turn off Hoymiles DC side (if DC side has power)
    if (hm_awake) {
    ...
    if (ShellyCommand(PM2_ADDR, PM_CONFIG)) ShellyCommand(PM2_ADDR, MW_OFF);
    CheckErrors();
...
void loop() {
    ...
    FinishCycle();  // while new power setting takes effect: update energy counters, carry out maintenance tasks, flash LED, read PV power
    CheckErrors();  // check for comms errors, halt system if an error is persistent
    UserIO();  // read user command, print cycle info and user command response
}
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (!strcmp(shelly_command, PM_STATUS)) {
            ...
            if (shelly_addr == PM2_ADDR) {
                ...
                if (power_new > 0) power_ess = power_ess*PM2_MW_POWER_CORR;
            }
        }
        if (!strcmp(shelly_command, MW_ON)) {
            // Meanwell turned on
            ts_MW = millis();
            delay(1000);  // allow a little more time for power output to stabilize
            if (!mw_on) mw_counter++;
            mw_on = true;
        }
        if (!strcmp(shelly_command, MW_OFF)) {
            // Meanwell turned off
            ts_MW = millis();
            delay(1000);  // allow a little more time for power to reach zero
            if (mw_on) mw_counter++;
            mw_on = false;
        }
        if (!strcmp(shelly_command, HM_WAKEUP)) {
            // Hoymiles AC side turned on: turn on DC side, too
            ts_HM = millis();
            hm_awake = BMSCommand(DISCH_ON);
        }
        if (!strcmp(shelly_command, HM_SHUTDOWN)) {
            // Hoymiles AC side turned off: turn off DC side, too
            ts_HM = millis();
            hm_awake = !BMSCommand(DISCH_OFF);
        }
         ...
         if ((bms_resp[2]<<8|bms_resp[3]) == len-2) {  // response length
             if ((bms_resp[len-2]<<8|bms_resp[len-1]) == sum-bms_resp[len-2]-bms_resp[len-1]) {  // response checksum
                ...
                if (bms_resp[RS485_COMMAND_POS] == READ_ALL) {
                    ...
                    if (!hm_awake) hm_limit = 0;  // BMS discharging switch turned off during setup? Then prevent any attempt of discharging
                    return true;
...
void FinishCycle() {
    ...
    if (((millis()-ts_HM)/1000 >= RF24_KEEPALIVE) && hm_awake) {
    ...
    if (!mw_on && ((millis()-ts_MW)/2000 >= PROCESSING_DELAY)) ledcWrite(PWM_OUTPUT_PIN, 0);
    ...
    if ((vcell_min <= ESS_UVP) && !hm_limit && hm_awake) ShellyCommand(PM2_ADDR, HM_SHUTDOWN);
    if ((vcell_min >= ESS_UVPR-20) && (power_new > 0) && !hm_awake) ShellyCommand(PM2_ADDR, HM_WAKEUP);  // wake up HM before reaching ESS_UVPR, to allow time for AC sync procedure
    ...
    // wait until power changes have almost stabilized
    while ((millis()-ts_power) < PROCESSING_DELAY*1000-100);
    // read PV power from Shelly 1PM (normally takes less than 100 ms)
    ShellyCommand(PM1_ADDR, PM_STATUS);
...
void CheckErrors() {
    ...
    error_msg[0] = '\0';  // assumption: no errors during next cycle
    ...
    while (true) {  // wait for user to confirm restart
        ...
        while ((millis()-ts_power)/2000 < PROCESSING_DELAY);  // wait for two PROCESSING_DELAYS
        if (telnet.available()) {
...
void UserIO() {
    ...
    if (!telnet) {
        ...
        telnet = telnet_server.available();  // new telnet session?
    }
    if (telnet) {
        if (telnet.available()) command = telnet.read();  // read user command
        PrepareCycleMsg();  // prepare cycle status message
        PrepareCmdResp();  // prepare user command reponse
        telnet.printf("%s%s", cycle_msg, cmd_resp);  // print cycle status message and user command response
    }
    if (!pm1_eco_mode || !pm2_eco_mode) while ((millis()-ts_power)/1000 < PROCESSING_DELAY);  // wait until power changes have completely stabilized
    else while ((millis()-ts_power)/2000 < PROCESSING_DELAY);  // ESS is asleep: wait for two PROCESSING_DELAYS
}
...
bool HoymilesCommand(int hm_command) {
    ...
    if ((hm_command == HM_POWER_OFF) || (hm_command == HM_POWER_ON)) {  // switch command
        ...
        ts_HM = millis();
        if (hm_command == HM_POWER_OFF) strcpy(error_msg, "RF24 Hoymiles turn off command failed");
...
void PrepareCycleMsg() {
    ...
    strcat(cycle_msg, " ");
    if (error_flag && (command != 'e')) strcat(cycle_msg, ERROR_SYMBOL);  // "unread error" symbol
    ...
    switch (command) {
        ...
        case 'e':
            ...
            for (int i=0; i<ERROR_TYPES; i++) {
                sprintf(cmd_resp + strlen(cmd_resp), "%-4s: %3d", ERROR_TYPE[i], error_counter[i]);
            ...
            strcat(cmd_resp, "\r\n");
            if (error_msg[0] != '\0') sprintf(cmd_resp + strlen(cmd_resp), "%s%s\r\n\n", ERROR_SYMBOL, error_msg);  // error has just occured: show ERROR_SYMBOL
            else if (last_error_msg[0] != '\0') sprintf(cmd_resp + strlen(cmd_resp), "Last: %s\r\n\n", last_error_msg);
            error_flag = false;
            break;
        case 's':
