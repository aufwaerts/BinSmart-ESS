...
void FinishCycle() {
    ...
    if (int(round(power_ess)) > 0) {
        if (power_grid_to_ess > power_pv_to_ess) cycle_msg += GRID_FLOW_SYMBOL[2 + (power_old >= mw_power_limit)];
        else cycle_msg += PV_FLOW_SYMBOL[2 + (power_old >= mw_power_limit)];
    }
    else  cycle_msg += ESS_FLOW_SYMBOL[(power_ess < 0) + 2*(power_old <= hm_power_limit)];
    cycle_msg += CABLE_SYMBOL[!odd_cycle];
    ...
    if (power_new != power_old) cycle_msg += DIFF_SYMBOL[(power_new < power_old) + rampdown];
