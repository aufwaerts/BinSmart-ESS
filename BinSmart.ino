...
void setup() {
    ...
    if (ReadBMSSettings()) telnet.print("RS485 communication with JKBMS OK\r\n");
    ...
    if (BMS_resp[0]<<8|BMS_resp[1] == 0x4E57) {  // start frame signature
        if (BMS_resp[2]<<8|BMS_resp[3] == len-2) {  // response length
            if (BMS_resp[len-2]<<8|BMS_resp[len-1] == sum) {  // response checksum
                ...
                int index = 0; while (BMS_resp[index] != BMS_VCELLS) index++;
...
bool ReadBMSSettings() {

    // Send command to BMS via RS485
    Serial2.write(BMS_READ_SETTINGS, sizeof(BMS_READ_SETTINGS));
    Serial2.flush();
    // Wait for response
    for (int i=0; i<1000; i++) if (Serial2.available()) break;
    ...
    if (BMS_resp[0]<<8|BMS_resp[1] == 0x4E57) {  // start frame signature
        if (BMS_resp[2]<<8|BMS_resp[3] == len-2) {  // response length
            if (BMS_resp[len-2]<<8|BMS_resp[len-1] == sum) {  // response checksum
                ...
                int index = 0; while (BMS_resp[index] != BMS_WARNINGS) index++;
                if (BMS_resp[index+1]<<8|BMS_resp[index+2]) {
                    error_msg = "Check BMS warnings";
                    return false;
                }
                ...
                if ((BMS_resp[index+1]<<8|BMS_resp[index+2]) <= CELL_OVP) {
                    error_msg = "CELL_OVP must be lower than BMS OVP";
                    return false;
                }
                ...
                if (((BMS_resp[index+1]<<8|BMS_resp[index+2]) >= CELL_UVP) || ((BMS_resp[index+1]<<8|BMS_resp[index+2]) >= CELL_UUVP)) {
                    error_msg = "CELL_UVP and CELL_UUVP must be higher than BMS UVP";
                    return false;
                }
    ...
    error_msg = "Could not read settings from BMS";
    return false;
}
