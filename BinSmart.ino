...
void loop() {
    ...
    while (millis()-ts_power < max(PROCESSING_DELAY+statechange_delay, (!uvp_countdown)*UVP_SLEEP_DELAY))
        if (UserCommand(READ)) break;  // max one command per cycle
    if (telnet) telnet.print("\033[?25l\u0013");  // hide cursor, disable telnet input
    while (millis()-ts_power < PROCESSING_DELAY+statechange_delay);  // if user command was processed, wait for end of PROCESSING_DELAY
}
...
void SetNewPower() {
    ...
    power_ess = power_new;
    // Assume no charging/discharging state change this cycle
    statechange_delay = 0;
    ...
    if (power_new == 0) {  // turn charging and discharging off
        ...
        if (power_ess != 0) statechange_delay = STATE_CHANGE_DELAY;
        if (power_ess > 0) mw_counter++;  // increase MW plug relay counter
    }
    if (power_new > 0) {  // set new charging power, turn discharging off, turn charging on
        ...
        if (power_ess <= 0) {
            statechange_delay = STATE_CHANGE_DELAY;
            mw_counter++;  // increase MW plug relay counter
        }
    }
    if (power_new < 0) {  // set new discharging power, turn charging off, turn discharging on
        HoymilesCommand(hm_power, sizeof(hm_power));
        ShellyCommand(MWPLUG_OFF);
        HoymilesCommand(hm_turnon, sizeof(hm_turnon));
        if (power_ess >= 0) statechange_delay = STATE_CHANGE_DELAY;
        if (power_ess > 0) mw_counter++;  // increase MW plug relay counter
    }
    ts_power = millis();
...
void CheckErrors() {
    ...
    while (true) {
        ...
        FlashLED(false);
        ts_cycle = millis();
        while (millis()-ts_cycle < PROCESSING_DELAY+statechange_delay) UserCommand(READ);
    }
}
