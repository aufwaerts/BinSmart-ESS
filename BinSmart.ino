...
void setup() {
    ...
    cycle_msg = "ESS Started";  // indicates first cycle
    delay(1000);
    ...
    if (!ShellyCommand(EM_ADDR, EM_STATUS)) telnet.println(ERROR_SYMBOL + error_msg);
                ...
                if (BMS_resp[RS485_COMMAND_POS] == READ_DATA) {
                    if (BMS_resp[DATA_ID_POS] == VCELLS_ID) {
                        ...
                        if (vcell_max >= ESS_OVP) {  // OVP cell voltage reached
                            if (mw_limit <= MW_MIN_POWER) mw_limit = 0;  // enter (or remain in) OVP mode
                        ...
                        if (vcell_min <= ESS_UVP) {  // UVP cell voltage reached
                            if (hm_limit >= HM_MIN_POWER) hm_limit = 0;  // enter (or remain in) UVP mode
...
void FinishCycle() {
    ...
    if (cycle_msg != "ESS started") {  // skip energy counters during first cycle
    ...
    if (millis()-ts_HM >= RF24_KEEPALIVE) HoymilesCommand(power_new < 0);
    ...
    if ((power_pv < MW_MIN_POWER) && !hm_limit && !power_old && !power_new && !pm2_eco_mode) pm2_eco_mode = ShellyCommand(PM2_ADDR, PM_ECO_ON);
...
void ProcessUserCommand() {
    ...
    switch (command) {
        ...
        case 'n':
            ...
            // PV energy wasted to ESS (AC/DC/AC conversion losses of PV energy sent to ESS instead of consumer)
            if (en_from_pv > 0.1) {
                cmd_resp += "PV to ESS wasted: ";
                sprintf(buf,"%.1f %%\r\n",(en_pv_wasted)/en_from_pv*100);
                cmd_resp += buf;
...
bool UpdateDDNS() {
    ...
    if (http_resp_code == HTTP_CODE_OK) {
        ...
        http.end();
        error_msg = "DDNS update command failed (code ";
        error_msg += http_resp_code;
        error_msg += ")";
        return false;
    }
