...
#include <Dusk2Dawn.h>  // https://github.com/dmkishi/Dusk2Dawn, PROPERTIES _latitude, _longitude and _timezone MADE PUBLIC
...
void setup() {
    ...
    // Check status of Meanwell Shelly plug, turn plug off
    if (ShellyCommand(MWPLUG_STATUS)) telnet.print("Meanwell Shelly plug found\r\n");
    if (mw_turned_on) ShellyCommand(MWPLUG_OFF);

    // Check status of Hoymiles Shelly plug, turn on/off, depending on lowest cell voltage
    if (ShellyCommand(HMPLUG_STATUS)) telnet.print("Hoymiles Shelly plug found\r\n");
    if (hm_turned_on && (vcell_min <= ESS_UVP)) ShellyCommand(HMPLUG_OFF);
    if (!hm_turned_on) {
        if (vcell_min >= ESS_UVPR) ShellyCommand(HMPLUG_ON);
        else hm_power_limit = 0;  // Enter UVP mode (disable discharging)
    }

     // Read Shelly 1PM (PV) power
...
void loop() {
    ...
    while (millis()-ts_power < max(PROCESSING_DELAY, (!uvp_countdown)*UVP_SLEEP_DELAY))
             ...
             if ((BMS_resp[len-2]<<8|BMS_resp[len-1]) == sum-BMS_resp[len-2]-BMS_resp[len-1]) {  // response checksum
                ...
                if (command[BMS_COMMAND_POS] == BMS_READ_ALL) {
                    ...
                    if (BMS_resp[BMS_DATA_ID_POS] != BMS_VCELLS_ID) {
                        error_msg = "Could not read BMS cell voltages";
                        return false;
                    }
                    vbat = vcell_max = 0; vcell_min = 4000;
                    for (int vcells_pos = BMS_DATA_ID_POS+3; vcells_pos <= BMS_DATA_ID_POS+BMS_resp[BMS_DATA_ID_POS+1]; vcells_pos+=3) {
...
bool ShellyCommand(const String URL) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (URL == MWPLUG_STATUS) {
            mw_turned_on = (http_resp.indexOf("\"output\":true") > -1);
            return true;
        }
        if (URL == HMPLUG_ON) {
            ts_HM_ON = millis();
            if (!hm_turned_on) hm_counter++;
            hm_turned_on = true;
            return true;
        }
        if (URL == HMPLUG_OFF) {
        ...
        if (URL == HMPLUG_STATUS) {
            hm_turned_on = (http_resp.indexOf("\"output\":true") > -1);
            return true;
        }
...
void SetNewPower() {
    ...
    // Save power setting from previous cycle as baseline for new power calculation
    power_ess = power_new;
    ...
    if (auto_recharge) power_new = max(MW_RECHARGE_POWER, power_new);  // Auto recharge mode overrides calculated and manual setting
    ...
    if (power_ess - power_new > POWER_RAMPDOWN_RATE) {
        if (power_ess > MW_MIN_POWER) {
            power_new = max(power_ess - POWER_RAMPDOWN_RATE, MW_MIN_POWER);  // MW above min power: ramp down power immediately
            filter_cycles = 0;  // expire filter cycle countdown immediately
            if (power_new == MW_MIN_POWER) filter_cycles = POWER_FILTER_CYCLES;  // MW at min power: reset filter cycle countdown
        }
        else {
            filter_cycles = max(filter_cycles-1, 0);  // countdown filter cycles
            if (filter_cycles) power_new = power_ess;  // filter out power spikes
            else power_new = power_ess - POWER_RAMPDOWN_RATE;  // ramp down ESS power after filtering out power spikes
        }
    }
    ...
    if (power_n == 0) {  // turn charging or discharging off
        if (power_ess <= 0) if (!HoymilesCommand(hm_turnoff, sizeof(hm_turnoff))) power_new = power_ess;  // repeat command every cycle, just to make sure HM is really turned off
        if (power_ess > 0) if (!ShellyCommand(MWPLUG_OFF)) power_new = power_ess;
    }
    if (power_n > 0) {  // set new charging power
        if (power_new != power_ess) SetMWPower();  // set new PWM value only if power setting has changed
        if (power_ess == 0) if (!ShellyCommand(MWPLUG_ON)) power_new = 0;
        if (power_ess < 0) {
            if (!HoymilesCommand(hm_turnoff, sizeof(hm_turnoff))) power_new = power_ess;
    ...
    if (power_n < 0) {  // set new discharging power
        if (!HoymilesCommand(hm_power, sizeof(hm_power))) power_new = power_ess;  // send HM power setting every cycle, even if power remains unchanged
        else {
            if (power_ess == 0) {
            ...
            if (power_ess > 0) {
                if (!ShellyCommand(MWPLUG_OFF)) power_new = power_ess;  // takes longer than RF24_MIN_DELAY
...
void FinishCycle() {
    ...
    // If in UVP mode, decrease uvp_countdown
    if (!hm_power_limit) {  // UVP mode (HM disabled)
        if (daytime) uvp_countdown = max(uvp_countdown-1, 0);  // daytime: decrease uvp_countdown by 1
        else uvp_countdown = max(uvp_countdown-(UVP_WAKEUP_RESET/2), 0);  // nighttime: decrease uvp_countdown rapidly
    }
    
    // MW is charging or HM is discharging or PV is producing: reset UVP countdown
    if (power_new || power_pv) uvp_countdown = UVP_WAKEUP_RESET;

    // UVP sleep mode: turn off Hoymiles AC side
    if (!uvp_countdown && hm_turned_on) ShellyCommand(HMPLUG_OFF);
    ...
    if (telnet) {
        ...
        if ((power_ess > hm_power_limit) && (power_ess < mw_power_limit))
            cycle_msg += FLOW_SYMBOL[(power_ess<0)+2*(power_ess>0)];
        else {
            if (power_ess <= hm_power_limit)
                cycle_msg += FLOW_SYMBOL[3*(power_ess == HM_MAX_POWER) + 5*(power_ess > HM_MAX_POWER) + 2*(!power_ess)];
        ...
        if (vbat >= ESS_UVP*8) cycle_msg += BATT_LEVEL_SYMBOL[min(int(vbat-ESS_UVP*8)*(BATT_LEVELS-2)/((ESS_OVPR-ESS_UVP)*8)+1, BATT_LEVELS-1)];
        ...
        if (power_new != power_ess) {
            ...
            if (!filter_cycles) cycle_msg += RAMPDOWN_SYMBOL;
        }
        else if (filter_cycles < POWER_FILTER_CYCLES) cycle_msg += POWERFILTER_SYMBOL[filter_cycles%12];
    ...
    if ((power_new > 0) && ((millis()-ts_MW_ON)/1000 >= MW_PLUG_TIMER-30)) ShellyCommand(MWPLUG_ON);
    else {
        // Not in UVP mode: Keep alive Hoymiles AC-side (30 secs before plug timer expires)
        if (hm_power_limit && ((millis()-ts_HM_ON)/1000 >= HM_PLUG_TIMER-30)) ShellyCommand(HMPLUG_ON);
        else {
            // Periodically clear data held in internal 3EM memory (prevents http timeouts when EM reorgs internal memory)
            if (((millis()-ts_EM_reset)/1000 >= EM_RESET_INTERVAL) && (minute(unixtime) > 1)) ShellyCommand(EM_RESET);
            else {
                // Daytime: turn off Shelly 1PM eco mode
                if (daytime && pm_eco_mode) {
                    if (ShellyCommand(PM_ECO_MODE_OFF))
                        if (pm_eco_mode)
                    ...
                    if (!daytime && !pm_eco_mode) {
                        if (ShellyCommand(PM_ECO_MODE_ON))
                            if (!pm_eco_mode)
                                if (ShellyCommand(PM_REBOOT)) pm_eco_mode = true;
                    }
                    else UpdateDDNS();  // Check if public IP address was changed, update DDNS server entry
                }
            }
        }
    }

    // initiate "long polling" for EM_STATUS request in next cycle
    http.begin(EM_STATUS); http.connect();
}
...
bool UserCommand(bool read_input) {
    ...
    switch (command) {
        ...
        case 't':
            ...
            cmd_resp += "\r\nUTC offset   : ";
            if (dst) sprintf(buf,"%+d (DST)",ess_location._timezone + 1);
            else sprintf(buf,"%+d (ST)",ess_location._timezone);
            cmd_resp += buf;
            cmd_resp += "\r\nESS started  : ";
        ...
        case 's':
            ...
            cmd_resp += mw_counter;
            cmd_resp += " (";
            cmd_resp += String(mw_counter/(elapsedDays(unixtime-starttime)+1));
            cmd_resp += "/day)\r\n";
            cmd_resp += "Hoymiles plug: ";
            cmd_resp += hm_counter;
            cmd_resp += " (";
            cmd_resp += String(hm_counter/(elapsedDays(unixtime-starttime)+1));
            cmd_resp += "/day)\r\n\n";
            break;
        case 'z':
        ...
        default:
            ...
            cmd_resp += "[s] - Shelly plug counters\r\n";
...
bool HoymilesCommand(byte command[], int size) {
    ...
    if ((command[10] == 0x0B) && (power_new != power_ess)) {
