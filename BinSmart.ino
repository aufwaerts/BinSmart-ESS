...
bool ShellyCommand(IPAddress shelly_addr, const String shelly_command, int value = -1);
...
void SetNewPower() {
    ...
    if (power_new > 0) {  // set new charging power, (if necessary) turn discharging off, turn charging on
        ...
        if (power_old == 0) {
            ...
            if (!ShellyCommand(PM2_ADDR, PM_CH0_ON, MW_TIMER)) power_new = 0;
        }
        if (power_old < 0) {
            if (!HoymilesCommand(HM_OFF)) power_new = power_old;
            else {
...
bool ShellyCommand(IPAddress shelly_addr, const String shelly_command, int value) {
...
void FinishCycle() {
    ...
    if (mw_on && ((millis()-ts_MW)/1000 >= MW_TIMER-10*PROCESSING_DELAY)) ShellyCommand(PM2_ADDR, PM_CH0_ON,  MW_TIMER);
