                    ...
                    if (BMS_resp[BMS_DATA_ID_POS] == BMS_VCELLS_ID) {
                        ...
                        if (vcell_max >= ESS_OVP) {  // OVP cell voltage reached: reduce MW power limit
                            if (mw_power_limit == MW_MIN_POWER) mw_power_limit = 0;  // enter OVP mode
                                ...
                                if ((mw_power_limit < MW_MIN_POWER) && (mw_power_limit > 0)) mw_power_limit = MW_MIN_POWER;  // always last power limit before OVP
                            }
                        }
                        ...
                        if (vcell_min <= ESS_UVP) {  // UVP cell voltage reached: reduce HM power limit
                            if (hm_power_limit == HM_MIN_POWER) hm_power_limit = 0;  // enter UVP mode
                                ...
                                if ((hm_power_limit > HM_MIN_POWER) && (hm_power_limit < 0)) hm_power_limit = HM_MIN_POWER;  // always last power limit before UVP
                            }
                        }
...
bool UpdateDDNS() {
    ...
    if (millis()-ts_pubip < DDNS_UPDATE_INTERVAL) return true;
