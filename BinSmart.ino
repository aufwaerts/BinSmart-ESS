...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        cycle_msg += buf;
        if (!daytime) cycle_msg += PV_SYMBOL[2+(telnet.remoteIP()==WIN_CLIENT_ADDR)];
        else {
            if (power_pv < PV_SUN_THRESHOLD) cycle_msg += PV_SYMBOL[4+(telnet.remoteIP()==WIN_CLIENT_ADDR)];
            else cycle_msg += PV_SYMBOL[min(((power_pv-PV_SUN_THRESHOLD)*(PV_LEVELS/2-3)/(PV_MAX_POWER-PV_SUN_THRESHOLD)+3)*2+(telnet.remoteIP()==WIN_CLIENT_ADDR),PV_LEVELS-1)];
        }
