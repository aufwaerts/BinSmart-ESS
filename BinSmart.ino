...
void setup() {
    ...
    telnet_server.begin();
    while (!telnet) telnet = telnet_server.available();
...
void FinishCycle() {
    ...
    if (!mw_on && ((millis()-ts_MW)/2000 >= PROCESSING_DELAY)) ledcWrite(PWM_OUTPUT_PIN, PWM_DUTY_CYCLE_MIN);
    ...
    // wait until power changes have almost settled
...
void CheckErrors() {
    ...
    if (!HoymilesCommand(HM_OFF)) ShellyCommand(PM2_ADDR, HM_AC_OFF);  // if HM DC side can't be turned off: turn off AC side
    SetMWPower(MW_MIN_POWER);
    ShellyCommand(PM2_ADDR, MW_OFF);
    ledcWrite(PWM_OUTPUT_PIN, PWM_DUTY_CYCLE_MIN);  // turn off Meanwell PWM optocoupler
    ...
    while (true) {  // wait for user to confirm restart
        ...
        delay(PROCESSING_DELAY*1000-600);
        if (telnet.available()) {
...
void UserIO() {
    ...
    while ((millis()-ts_power)/1000 < PROCESSING_DELAY);  // wait until power changes have completely settled
}
...
void SetMWPower(int power) {
    ...
    ledcWrite(PWM_OUTPUT_PIN, pwm_duty_cycle);
}
