...
void FinishCycle() {
    ...
    // Set MW Plug eco mode (turned of at daytime)
    if (daytime && pm_eco_mode) ShellyCommand(PM_ECO_OFF);
    if ((!daytime) && (!pm_eco_mode)) ShellyCommand(PM_ECO_ON);
...
bool UpdateDDNS() {
    ...
    http.end();
    error_msg = "Could not obtain public DDNS address (http code ";
    error_msg += http_resp_code;
    error_msg += ")";
    return false;
}
