...
void setup() {
    ...
    // Turn on Hoymiles plug, reset energy counter
    ...
    // Read PV power from Shelly PM, reset energy counter
    if (ShellyCommand(PM_STATUS)) telnet.print("Shelly 1PM found\r\n");
    ShellyCommand(PM_RESET);

    // Read time values from Shelly 3EM, reset energy counters
    if (ShellyCommand(EM_STATUS)) telnet.print("Shelly 3EM found\r\n");
    ShellyCommand(EM_RESET);

    // No erros during setup: Set timestamps, zero error counters, start polling cycle
    if (error_msg == "") {
...
void loop() {
    ...
    BMSCommand(BMS_VOLTAGES, sizeof(BMS_VOLTAGES));  // Read battery cell voltages
    ...
    while (millis()-ts_power < PROCESSING_DELAY);  // if user command was processed, wait at least for end of PROCESSING_DELAY
}

bool BMSCommand(const byte command[], int size) {
    
    // minimum delay between two consecutive BMS commands: 100 ms
    while (millis()-ts_BMS < 100);
...
void SetNewPower() {
    ...
    if (power_new != power_old) {
        ...
        if (power_new < 0) {  // set new discharging power, (if necessary) turn charging off, turn discharging on
            if (power_old < 0)
                if (!HoymilesCommand(hm_power, sizeof(hm_power))) power_new = power_old;
            if (power_old == 0) {
                if (!HoymilesCommand(hm_power, sizeof(hm_power))) power_new = 0;
...
void CheckErrors() {
    ...
    while (true) {
        ...
        FlashLED(false);
        ts_power = millis();
        while (millis()-ts_power < PROCESSING_DELAY) UserCommand(READ);
    }
}
...
bool HoymilesCommand(byte command[], int size) {
    ...
    if (radio.writeBlocking(command, size, 1000))
