...
void SetNewPower() {
    ...
    if (auto_recharge) power_new = max(MW_RECHARGE_POWER, power_new);
    
    if (manual_mode) {
        power_new = power_manual;   // Manual mode: manual power setting overrides calculation
        ...
        if (power_ess - power_new > POWER_RAMPDOWN_RATE) {
            if (power_ess > 0) {
                ...
                filter_cycles = 0;  // consider filter cycle countdown expired
                if (power_new == 0) filter_cycles = POWER_FILTER_CYCLES;  // MW switched off: reset filter cycle countdown
            }
            else {
                filter_cycles = max(filter_cycles-1, 0);  // countdown filter cycles
                if (filter_cycles) power_new = power_ess;  // filter out power spikes
                else power_new = power_ess - POWER_RAMPDOWN_RATE;  // ramp down ESS power (i.e. ramp up HM power) after filtering out power spikes
            }
        }
...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        if ((filter_cycles < POWER_FILTER_CYCLES) && (power_new > hm_power_limit)) cycle_msg += POWERFILTER_SYMBOL[filter_cycles];
