                ...
                if (BMS_resp[RS485_COMMAND_POS] == READ_DATA) {
                    if (BMS_resp[DATA_ID_POS] == VCELLS_ID) {
                        ...
                        if (vcell_max >= ESS_OVP) {  // OVP cell voltage reached: reduce MW power limit
                            if (mw_limit == MW_MIN_POWER) mw_limit = 0;  // enter OVP mode
                            else {
                                mw_limit = round(power_new*POWER_LIMIT_RAMPDOWN);  // ramp down charging power limit softly
                                if ((mw_limit < MW_MIN_POWER) && (mw_limit > 0)) mw_limit = MW_MIN_POWER;  // always last power limit before OVP
                            }
                        }
                        ...
                        if (vcell_min <= ESS_UVP) {  // UVP cell voltage reached: reduce HM power limit
                            if (hm_limit == HM_MIN_POWER) hm_limit = 0;  // enter UVP mode
                            else {
                                hm_limit = round(power_new*POWER_LIMIT_RAMPDOWN);  // ramp up discharging power limit softly
                                if ((hm_limit > HM_MIN_POWER) && (hm_limit < 0)) hm_limit = HM_MIN_POWER;  // always last power limit before UVP
                            }
                        }
                        return true;
                    }
...
void FinishCycle() {
    ...
    if ((hm_limit == HM_MAX_POWER) && bms_bal_on) bms_bal_on = !BMSCommand(BAL_OFF);
}
