...
bool ShellyCommand(IPAddress shelly_addr, const char shelly_command[]) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (!strcmp(shelly_command, EM_STATUS)) {
            ...
            unixtime = unixtime_utc + (timezone+dst)*3600;  // adjust unixtime to match local time
...
void ProcessUserCommand() {
    ...
    switch (command) {
        ...
        case 'b':
            ...
            break;
        case 'd':
            sprintf(cmd_resp, "ESS public IP address: %s\r\nLast address check   : %02d/%02d/%04d %02d:%02d\r\n", public_IP, day(pubip_time), month(pubip_time), year(pubip_time), hour(pubip_time), minute(pubip_time));
...
bool UpdateDDNS() {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        http.getString().toCharArray(public_IP, sizeof(public_IP));
        http.end();
        ts_pubip = millis();
        pubip_time = unixtime;
        if (!strcmp(public_IP, DDNS_address)) return true;  // public IP unchanged

        strcpy(http_command, DDNS_SERVER_URL);
        strcat(http_command, public_IP);
        http.begin(http_command);  // update DDNS server entry
        if (http.GET() == HTTP_CODE_OK) {
            http.end();
            strcpy(DDNS_address, public_IP);
            DDNS_time = unixtime;
            return true;
        }
        http.end();
