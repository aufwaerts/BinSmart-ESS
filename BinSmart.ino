...
void setup() {
    ...
    // Turn off Meanwell plug, reset relay counter
    if (ShellyCommand(MWPLUG_OFF)) telnet.print("Meanwell Shelly plug found\r\n");
    mw_relay_counter = 0;
    mw_relay_off = true;

    // Turn on Hoymiles AC plug
    if (ShellyCommand(HMPLUG_ON)) telnet.print("Hoymiles Shelly plug found\r\n");
...
bool ShellyCommand(const String URL) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (URL == MWPLUG_ON) {
            ts_MW_ON = millis();
            if (mw_relay_off) mw_relay_counter++;
            mw_relay_off = false;
            return true;
        }
        if (URL == MWPLUG_OFF) {
...
void SetNewPower() {
    ...
    if (power_ess - power_new > POWER_RAMPDOWN_RATE) {
        if (power_ess > 0) {
            power_new = max(power_ess - POWER_RAMPDOWN_RATE, 0);  // MW is charging: ramp down power immediately
            filter_cycles = 0;  // expire filter cycle countdown immediately
            if (power_new == 0) filter_cycles = POWER_FILTER_CYCLES;  // MW turned off: restart filter cycle countdown
        }
        else {
...
bool UserCommand(bool read_input) {
    ...
    switch (command) {
        ...
        case 's':
            ...
            if (read_input) break;  // ... starting next cycle
            cmd_resp = "MW Shelly relay ops since ";
            sprintf(buf,"%02d/%02d/%04d %02d:%02d\r\n",day(starttime),month(starttime),year(starttime),hour(starttime),minute(starttime));
            cmd_resp += buf;
            cmd_resp += mw_relay_counter;
            cmd_resp += " (";
            cmd_resp += String(mw_relay_counter/(elapsedDays(unixtime-starttime)+1));
            cmd_resp += "/day)\r\n\n";
            break;
        case 'z':
