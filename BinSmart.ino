...
void setup() {
    ...
    if (vcell_min <= ESS_UVP) ShellyCommand(HMPLUG_OFF);
    else ShellyCommand(HMPLUG_ON);
...
bool UpdateDDNS() {
    ...
    // update interval passed: read public IP from server (alternate between primary and backup server)
    if (ts_pubip % 2) http.begin(PUBLIC_IP1);
    else http.begin(PUBLIC_IP2);
    if (http.GET() == HTTP_CODE_OK) {
    ...
    http.end();
    error_msg = "Could not obtain PUBIP address";
    return false;
}
