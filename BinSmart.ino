...
void loop() {
    ...
    http.begin(EM_STATUS); http.connect();  // "long polling" for EM_STATUS request in next cycle

    // Read user command while waiting for power changes to take effect (UVP sleep mode: extend waiting time)
    if (telnet) telnet.print("\033[?25h\u0011");  // show cursor, enable telnet input
    ...
    if (telnet) telnet.print("\033[?25l\u0013");  // hide cursor, disable telnet input
...
bool ShellyCommand(const String URL) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (URL == MWPLUG_ECO_MODE_OFF) {
            if (http_resp == "{\"restart_required\":false}") mwplug_eco_mode = false;
            return true;
        }
        if (URL == MWPLUG_ECO_MODE_ON) {
            if (http_resp == "{\"restart_required\":false}") mwplug_eco_mode = true;
            return true;
        }
        if (URL == MWPLUG_REBOOT) {
            delay(4000); // wait for Shelly 1PM to reboot
            return true;
        }
        if (URL == HMPLUG_ON) {
...
void SetNewPower() {
    ...
    if (vcell_min <= ESS_UVP) {
        hm_power_limit = round(power_new*POWER_LIMIT_RAMPDOWN);  // ramp down discharging power limit softly
...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        if ((power_ess > hm_power_limit) && (power_ess < mw_power_limit)) {
            ...
            if (power_ess >= mw_power_limit) {
                if (power_grid_to_ess > power_pv_to_ess) cycle_msg += GRID_FLOW_SYMBOL[4*(power_ess >= mw_max_power) + 6*(power_ess < mw_max_power) + 2*(!power_ess)];
                else cycle_msg += PV_FLOW_SYMBOL[4*(power_ess >= mw_max_power) + 6*(power_ess < mw_max_power) + 2*(!power_ess)];
            }
            else cycle_msg += ESS_FLOW_SYMBOL[3*(power_ess == HM_MAX_POWER) + 5*(power_ess > HM_MAX_POWER) + 2*(!power_ess)];
        }
    ...
    // if (hm_power_limit && ((millis()-ts_HM_ON)/1000 >= HM_PLUG_TIMER-30)) { ShellyCommand(HMPLUG_ON); return; }
    ...
    // PV production above MW_MIN_POWER: turn off Shelly MW plug eco mode
    if ((power_pv >= MW_MIN_POWER) && mwplug_eco_mode) {
        if (ShellyCommand(MWPLUG_ECO_MODE_OFF))
            if (mwplug_eco_mode)
                if (ShellyCommand(MWPLUG_REBOOT)) mwplug_eco_mode = false;
        return;
    }

    // Nighttime: turn on Shelly MW plug eco mode
    if (!daytime && !mwplug_eco_mode) {
        if (ShellyCommand(MWPLUG_ECO_MODE_ON))
            if (!mwplug_eco_mode)
                if (ShellyCommand(MWPLUG_REBOOT)) mwplug_eco_mode = true;
        return;
    }
...
bool UpdateDDNS() {
    ...
    if (millis()%2) http.begin(PUBLIC_IP1);
