...
void SetNewPower() {
    ...
    // Update OVP max charging power, depending on max cell voltage
    if (vcell_max <= ESS_OVPR) mw_power_limit = MW_MAX_POWER;  // exit OVP mode or update Meanwell power limit
    ...
    if (power_new != power_old) {
        ...
        if (power_new > 0) {  // set new charging power, (if necessary) turn discharging off, turn charging on
            if (power_old > 0) {
                if (!SetMWPower(power_new)) power_new = MW_MAX_POWER;
            }
            if (power_old == 0) {
                if (!SetMWPower(power_new)) power_new = MW_MAX_POWER;
...
void FinishCycle() {
    ...
    // MW limit symbol
    if (power_old == mw_power_limit) cycle_msg += MW_LIMIT_SYMBOL[odd_cycle];
    cycle_msg += "\r\n";
    ...
    cycle_msg += CABLE_SYMBOL[odd_cycle];
    ...
    // House symbol, HM limit symbol
    cycle_msg += HOUSE_SYMBOL;
    if (power_old == hm_power_limit) cycle_msg += HM_LIMIT_SYMBOL[odd_cycle];
    cycle_msg += "\r\n";
            ...
            while ((millis()-ts)/1000 < READCOMMAND_TIMEOUT)
                if (telnet.available()) {
                    ...
                    if ((input[0] == '0') || input.toInt()) {
                        ...
                        if (power_manual > mw_power_limit) power_manual = mw_power_limit;
                        if (power_manual < hm_power_limit) power_manual = hm_power_limit;
            ...
            while ((millis()-ts)/1000 < READCOMMAND_TIMEOUT)
                if (telnet.available()) {
                    ...
                    if (input[0] == 'n') {
                        ...
                        en_to_cons = 0;  // Reset consumed energy counter
...
bool SetMWPower(float power) {

    float current = power/vbat;
    int pwm_duty_cycle;
    // translate power setting to PWM value
    if (power >= MW_LOW_POWER_THRESHOLD) pwm_duty_cycle = round(MW_POWER_FORMULA);
    else pwm_duty_cycle = round(MW_LOW_POWER_FORMULA);  //  compensate non-linear power output in low power range

    if (pwm_duty_cycle > PWM_DUTY_CYCLE_MAX) {
        pwm_duty_cycle = PWM_DUTY_CYCLE_MAX;
        ledcWrite(PWM_CHANNEL, pwm_duty_cycle);
        return false;  // power set to lowest possible output
    }
    if (pwm_duty_cycle < PWM_DUTY_CYCLE_MIN) {
        pwm_duty_cycle = PWM_DUTY_CYCLE_MIN;
        ledcWrite(PWM_CHANNEL, pwm_duty_cycle);
        return false;  // power set to physical power limit
    }
    ledcWrite(PWM_CHANNEL, pwm_duty_cycle);
    return true;  // power set as requested
}
