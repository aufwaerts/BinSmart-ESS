...
void setup() {
    ...
    if (!http_resp.reserve(1500)) error_msg = "String memory allocation failed";
    CheckErrors();
    delay(1000);
    ...
    if (BMSCommand(READ_SETTINGS)) BMSCommand(READ_VOLTAGES);
    CheckErrors();
    ...
    if (ShellyCommand(PM2_ADDR, PM_CONFIG))
        if (ShellyCommand(PM2_ADDR, PM_CH0_OFF)) ShellyCommand(PM2_ADDR, PM_CH1_ON);
    CheckErrors();
...
void loop() {
    ...
    ShellyCommand(EM_ADDR, EM_STATUS);  // Read time and AC grid power from Shelly 3EM
             ...
             if ((bms_resp[len-2]<<8|bms_resp[len-1]) == sum-bms_resp[len-2]-bms_resp[len-1]) {  // response checksum
                ...
                if (bms_resp[RS485_COMMAND_POS] == READ_DATA) {
                    ...
                    if (bms_resp[DATA_ID_POS] == CURRENT_ID) {
                        ...
                        pbat = (vbat/1000.0f)*(cbat/100.0f);
                        // Update batt charge level
                        int vbat_idle = vbat-round(cbat/16.0f);  // voltage at cbat=0
...
bool ShellyCommand(IPAddress shelly_addr, const String shelly_command) {
    ...
    // Put together Shelly http command
    String http_command = "http://";
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (shelly_command == EM_STATUS) {
            // read power and time from Shelly 3EM
            ...
            String local_time = http_resp.substring(index+7, index+12);  // local_time includes timezone and DST
            long utc_unixtime = atol(&http_resp[index+25]);  // read epoch time (UTC) from http response
            int utc_offset = (local_time.toInt()+24-hour(utc_unixtime))%24;  // utc_offset = timezone_offset + DST_offset
            unixtime = utc_unixtime + utc_offset*3600;  // adjust unixtime to match local_time
            ...
            if ((local_time == GET_ASTRO_TIME) || !starttime) {  // Calculate sunrise/sunset times for current day
                int sr_mins = ess_location.sunrise(year(unixtime), month(unixtime), day(unixtime), dst);
                sprintf(&sunrise[0], "%02d:%02d", sr_mins/60, sr_mins%60);
                int ss_mins = ess_location.sunset(year(unixtime), month(unixtime), day(unixtime), dst);
                sprintf(&sunset[0], "%02d:%02d", ss_mins/60, ss_mins%60);
            }
            daytime = ((local_time >= sunrise) && (local_time < sunset));  // set daytime/nighttime flag
            return true;
        }
        if ((shelly_command == PM_CH0_STATUS) || (shelly_command == PM_CH1_STATUS)) {
            // read power from Shelly 1PM or 2PM
            float power_reading = atof(&http_resp[http_resp.indexOf("apower")+8]);
        ...
        if (shelly_command == PM_CONFIG) {
            bool eco_mode = (http_resp[http_resp.indexOf("eco_mode")+10] == 't');
...
void FinishCycle() {
    ...
    // AC energy counters
    double hrs_cycle = secs_cycle/3600000.0;
    ...
    sprintf(buf,"%02d:%02d:%02d %+.3f",hour(unixtime),minute(unixtime),second(unixtime),secs_cycle/1000.0f);
    cycle_msg += buf;
    cycle_msg += OPS_SYMBOL[!power_new + (pm1_eco_mode && pm2_eco_mode)];
    cycle_msg += "\r\n";

    // OVP symbol above battery symbol
    cycle_msg += BAT_OVP_SYMBOL[(mw_limit < MW_MAX_POWER) + !mw_limit];
    ...
    cycle_msg += BAT_UVP_SYMBOL[(hm_limit > HM_MAX_POWER) + !hm_limit];
    ...
    if (mw_on && (millis()-ts_MW >= MW_TIMER-10*PROCESSING_DELAY)) ShellyCommand(PM2_ADDR, PM_CH0_ON);
    // Turn off Meanwell PWM optocoupler (if Meanwell is turned off)
    if (!mw_on && (millis()-ts_MW >= 3*PROCESSING_DELAY)) ledcWrite(PWM_CHANNEL, 0);
    ...
    if (millis()-ts_pubip >= DDNS_UPDATE_INTERVAL) UpdateDDNS();
}
...
void CheckErrors() {
    ...
    ShellyCommand(PM2_ADDR, PM_CH1_OFF);
    
    while (true) {
        if (millis()-ts_pubip >= DDNS_UPDATE_INTERVAL) UpdateDDNS();  // keep updating DDNS in order to be reachable via Internet
        cycle_msg = CLEAR_SCREEN;
        cycle_msg += SHOW_CURSOR;
        cycle_msg += "BinSmart ESS ";
        cycle_msg += SW_VERSION;
...
void ProcessUserCommand() {
    ...
    switch (command) {
        ...
        case 'b':
            ...
            cmd_resp += String(vbat/1000.0f,3);
            cmd_resp += " V\r\nCell voltages   : ";
            cmd_resp += vcell_min;
            cmd_resp += " - ";
            cmd_resp += vcell_max;
        ...
        case 's':
            ...
            cmd_resp += mw_counter;
            cmd_resp += " (";
            cmd_resp += mw_counter/(elapsedDays(unixtime-starttime)+1);
            cmd_resp += "/day)\r\n\n";
            break;
        case 'z':
            command = 0;  // respond to command only once
