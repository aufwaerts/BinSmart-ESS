...
void setup() {
    ...
    cycle_msg.reserve(300);
    cmd_resp.reserve(700);
    output_msg.reserve(1000);
    ...
    output_msg += "BinSmart ESS ";
    output_msg += SW_VERSION;
    output_msg += "\r\n\n";
    telnet.print(output_msg);
    delay(1000);
    output_msg = "WiFi connected to ";
    output_msg += WIFI_SSID;
    output_msg += "   RSSI: ";
    output_msg += WiFi.RSSI();
    output_msg += "   TxPower: ";
    output_msg += WiFi.getTxPower();
    output_msg += "\r\n";
    telnet.print(output_msg);
    delay(1000);
...
void loop() {
    ...
    if (!pm1_eco_mode) ShellyCommand(PM1_STATUS);  // Read PV power from Shelly 1PM
    if (power_new > 0) ShellyCommand(PM2_MW_STATUS);  // Read AC charging power from Shelly 2PM
    if (power_new < 0) ShellyCommand(PM2_HM_STATUS);  // Read AC discharging power from Shelly 2PM
    ...
    telnet.print(HIDE_CURSOR);  // discourage user input while system is in processing loop
}
...
void SetNewPower() {
    ...
    // Nighttime, low battery and no charging or discharging: enable sleep mode
    uvp_sleep_mode = !daytime && !hm_power_limit && !power_old && !manual_mode && !auto_recharge;
    ...
    if (power_new != power_old) {
        if (power_new == 0) {  // turn charging or discharging off
            ...
            if (power_old > 0)
                if (!ShellyCommand(PM2_MW_OFF)) power_new = power_old;
        }
...
void FinishCycle() {
    ...
    float power_pv_to_cons = min(power_cons, power_pv);
    float power_pv_to_ess = min(power_to_ess, power_pv - power_pv_to_cons);
    ...
    // Reset ESS power (will be measured at start of next cycle)
    power_ess = 0;

    // Keep alive RF24 module of Hoymiles inverter
    if ((millis()-ts_HM)/1000 >= RF24_KEEPALIVE) {
        if (power_new < 0) HoymilesCommand(HM_ON);
        else HoymilesCommand(HM_OFF);
    }

    // Maintenance tasks (maximum of one task per cycle to avoid overrun of PROCESSING_DELAY)
    ...
    // Set Shelly 2PM eco mode (turn off when PV produces more than MW min power, turn on when ESS is asleep)
    if (!uvp_sleep_mode && pm2_eco_mode) {
...
void CheckErrors() {
    ...
    output_msg += buf;
    output_msg += "\r\nafter ";
    output_msg += ERROR_LIMIT;
    output_msg += " consecutive errors\r\n\nLast error:\r\n";
    output_msg += ERROR_SYMBOL;
    output_msg += " ";
    output_msg += error_msg;
    output_msg += "\r\n\nEnter [r] to restart: ";
...
bool ReadCommand() {
    if (!telnet) {
        ...
        telnet.print(output_msg);  // welcome new telnet session with cycle message
        return false;
    }
...
void UserCommand() {
    ...
    switch (command) {
        case 'm':
            ...
            output_msg = cycle_msg;
            output_msg += "Enter ESS power (negative for discharging): ";
            telnet.print(output_msg);
            ts = millis();
        ...
        case 'g':
            command = 0;  // respond to command only once
            output_msg = cycle_msg;
            output_msg += "Enter grid power target: ";
            telnet.print(output_msg);
            ts = millis();
            while ((millis()-ts)/1000 < READCOMMAND_TIMEOUT)
                if (telnet.available()) {
        ...
        case 'z':
            command = 0;  // respond to command only once
            output_msg = cycle_msg;
            output_msg += "Enter [e] or [n] to reset error or energy stats: ";
            telnet.print(output_msg);
            ts = millis();
            while ((millis()-ts)/1000 < READCOMMAND_TIMEOUT)
                if (telnet.available()) {
        ...
        case 'r':
            command = 0;  // respond to command only once
            output_msg = cycle_msg;
            output_msg += "Enter [y] to confirm system reboot: ";
            telnet.print(output_msg);
            ts = millis();
            while ((millis()-ts)/1000 < READCOMMAND_TIMEOUT)
                if (telnet.available()) {
