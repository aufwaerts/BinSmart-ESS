...
void setup() {
    ...
    mwplug_eco_mode = daytime;  // assumption: wrong eco mode setting
    if (daytime) {
        ShellyCommand(MWPLUG_ECO_MODE_OFF);
        if (mwplug_eco_mode) ShellyCommand(MWPLUG_REBOOT);
        mwplug_eco_mode = false;
    }
    else {
        ShellyCommand(MWPLUG_ECO_MODE_ON);
        if (!mwplug_eco_mode) ShellyCommand(MWPLUG_REBOOT);
    ...
    pm_eco_mode = daytime;  // assumption: wrong eco mode setting
    if (daytime) {
        ShellyCommand(PM_ECO_MODE_OFF);
        if (pm_eco_mode) ShellyCommand(PM_REBOOT);
        pm_eco_mode = false;
    }
    else {
        ShellyCommand(PM_ECO_MODE_ON);
        if (!pm_eco_mode) ShellyCommand(PM_REBOOT);
        pm_eco_mode = true;
    }
...
bool ShellyCommand(const String URL) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (URL == MWPLUG_REBOOT) {
            delay(4000); // wait for Shelly plug to reboot
            return true;
        }
        if (URL == EM_SETTINGS) {
