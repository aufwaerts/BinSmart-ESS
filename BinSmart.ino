...
void loop() {
    ...
    RepeatCommand();  // Repeat command from previous cycle (if command was to be repeated)
    ...
    while (millis()-ts_power < max(PROCESSING_DELAY-NO_COMMAND_TIMER, (!uvp_countdown)*UVP_SLEEP_DELAY))
        if (ReadUserCommand()) break;  // read user command (maximum of one command per cycle)
...
void RepeatCommand() {
    if (repeat_command != ' ') ReadCommand(false);
}

bool ReadUserCommand() {
