...
bool ReadCommand(bool keyboard) {
    ...
    switch (command) {
        ...
        case 'n':
            ...
            cmd_resp += datetime_string;
            // from PV
            if (from_pv < 100) cmd_resp += " ";
            if (from_pv < 10) cmd_resp += " ";
            if (!from_pv) cmd_resp += "    ";
            cmd_resp += "\033[7m";  // highlight text
            if (!from_pv) cmd_resp += '0';
            else cmd_resp += String(from_pv,3);
            cmd_resp += "\033[27m";  // end text highlight
            ...
            cmd_resp += "\r\n";
            sprintf(&percent_string[0],"%5.1f %%",pv_to_grid/from_pv*100);
            cmd_resp += percent_string;
            ...
            cmd_resp += HOUSE_SYMBOL;
            cmd_resp += "\r\n";
            if (from_grid < 100) cmd_resp += " ";
            if (from_grid < 10) cmd_resp += " ";
            if (!from_grid) cmd_resp += "    ";
            cmd_resp += "\033[7m";  // highlight text
            if (!from_grid) cmd_resp += '0';
            else cmd_resp += String(from_grid,3);
            cmd_resp += "\033[27m";  // end text highlight
            cmd_resp += GRID_SYMBOL;
            cmd_resp += FLOW_SYMBOL[2];
            cmd_resp += GRID_CABLE_SYMBOL;
            ...
            cmd_resp += HOUSE_SYMBOL;
            cmd_resp += "\r\n";
            sprintf(&percent_string[0],"%5.1f %%",ess_to_grid/from_ess*100);
            cmd_resp += percent_string;
            cmd_resp += GRID_SYMBOL;
            cmd_resp += FLOW_SYMBOL[1];
            cmd_resp += GRID_CABLE_SYMBOL;
            ...
            else cmd_resp += String(from_ess,3);
            cmd_resp += "\033[27m";  // end text highlight
            // ESS efficiency
            cmd_resp += "\r\n\r\nFrom/to ESS: ";
