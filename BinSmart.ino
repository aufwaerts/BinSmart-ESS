...
bool ShellyCommand(const String URL) {
    ...
    if (http.GET() == HTTP_CODE_OK) {
        ...
        if (URL == EM_STATUS) {
            ...
            power_grid = round(atof(&http_resp[http_resp.indexOf("total_power",750)+13]));
            return true;
        }
        if (URL == MWPLUG_ON) {
            ts_MW_ON = millis();
...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        if (cmd_resp.substring(0,7) != "Command") cycle_msg += " or [o] for options";
        cycle_msg += ": ";
        telnet.print(cycle_msg);
    }
...
void SetMWPower() {
    ...
    if (power < MW_LINEAR_MIN) power = MW_LOW_POWER_CORRECTION;
    // translate power setting into PWM value
    int duty_cycle = round(MW_PWM_POWER_FORMULA);
...
bool HoymilesCommand(byte command[], int size) {
    ...
    if ((command[10] == 0x0B) && (power_new != power_old)) {
        ...
        int power = power_new;
        uint16_t limit = -10*round(HM_POWER_CORRECTION);  // HM command works with positive power value in deciwatts
