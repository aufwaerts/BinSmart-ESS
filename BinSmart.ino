...
void SetNewPower() {
    ...
    if (power_new == 0) {  // turn charging and discharging off
        // if ((cbat < 0) || ((millis()-ts_HM)/1000 >= RF24_TIMER)) HoymilesCommand(hm_turnoff, sizeof(hm_turnoff));
    ...
    if (power_new > 0) {  // set new charging power, (if necessary) turn discharging off, turn charging on
        if (power_new != power_old) SetMWPower();
        // if ((cbat < 0) || ((millis()-ts_HM)/1000 >= RF24_TIMER)) HoymilesCommand(hm_turnoff, sizeof(hm_turnoff));
        if (cbat < 0) HoymilesCommand(hm_turnoff, sizeof(hm_turnoff));
    ...
    if (power_new < 0) {  // set new discharging power, (if necessary) turn charging off, turn discharging on
        // if ((power_new != power_old) || ((millis()-ts_HM)/1000 >= RF24_TIMER)) HoymilesCommand(hm_power, sizeof(hm_power));
        if (power_new != power_old) HoymilesCommand(hm_power, sizeof(hm_power));
        if (cbat > 0) ShellyCommand(MWPLUG_OFF);
        // if ((cbat >= 0) || ((millis()-ts_HM)/1000 >= RF24_TIMER)) HoymilesCommand(hm_turnon, sizeof(hm_turnon));
...
void FinishCycle() {
    ...
    if (telnet) {
        ...
        cycle_msg += buf;
        cycle_msg += OPS_SYMBOL[!power_old + !hm_power_limit*!daytime];
        cycle_msg += "\r\n";
...
bool HoymilesCommand(byte command[], int size) {
    ...
    if (radio.writeFast(command, size))
        if (radio.txStandBy(RF24_TX_TIMEOUT, true)) {
            ts_HM = millis();
            return true;
        }
