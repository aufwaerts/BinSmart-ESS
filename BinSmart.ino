...
void setup() {
    ...
    // Read time values from Shelly 3EM, turn eco mode off
    if (ShellyCommand(EM_STATUS)) telnet.print("Shelly 3EM found\r\n");
    ShellyCommand(EM_ECO_OFF);
    delay(1000);

    // Read PV power from Shelly PM, turn eco mode off
    ...
    // Turn off Meanwell plug, turn eco mode off
...
void FinishCycle() {
    ...
    // Set Shelly 3EM eco mode (turned off when system is awake, turned on when system is asleep)
    if (!uvp_sleep_mode && em_eco_mode) {
        ShellyCommand(EM_ECO_OFF);
        delay(8000);  // wait for Shelly 3EM to reboot
        return;
    }
    if (uvp_sleep_mode && !em_eco_mode) {
        ShellyCommand(EM_ECO_ON);
        delay(8000);  // wait for Shelly 3EM to reboot
        return;
    }
