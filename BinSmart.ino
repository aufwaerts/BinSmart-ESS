...
void setup() {
    ...
    do {
        FlashLED(false); delay(PROCESSING_DELAY*1000);   // if WiFi unavailable or wrong SSID/PWD, system keeps flashing LED in endless loop
...
void FinishCycle() {
    ...
    if ((millis()-ts_HM)/1000 >= RF24_KEEPALIVE) HoymilesCommand(power_new < 0);
    ...
    if (!daytime && !pm1_eco_mode) pm1_eco_mode = ShellyCommand(PM1_ECO_ON);
