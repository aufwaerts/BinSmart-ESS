...
void loop() {
    ...
    BMSCommand(BMS_CURRENT, sizeof(BMS_CURRENT));  // Read DC charging/discharging current from BMS
...
bool BMSCommand(const byte command[], int size) {
    ...
    while (millis()-ts_BMS < 100);  // minimum delay between BMS packets: 100 ms
...
void SetNewPower() {
    ...
    power_old = power_new;  // Save power setting from previous cycle as baseline for new power calculation
    ...
    if (power_new != power_old) {
        if (power_new == 0) {  // turn charging or discharging off
            ...
            if (power_old > 0)
                SetMWPower(0); delay(100);  // prolongs relay lifetime
...
void FinishCycle() {
    ...
    en_pv_wasted += 0.2*hrs_cycle*min(power_from_grid, power_pv_to_ess);  // PV energy lost in ESS, due to AC/DC/AC conversion losses of ~20%
    ...
    // PV power
    sprintf(buf,"%4d",int(round(power_pv)));
    cycle_msg += buf;
    if (!daytime) cycle_msg += MOON_SYMBOL;
    else cycle_msg += SUN_SYMBOL[int(round(power_pv)) >= PV_MAX_POWER];
    cycle_msg += PV_FLOW_SYMBOL[(int(round(power_pv)) > 0) + 3*(int(round(power_pv)) >= PV_MAX_POWER)];
    cycle_msg += PV_CABLE_SYMBOL[int(round(power_pv)) >= PV_MAX_POWER];

    // ESS power
    if (int(round(power_ess)) > 0) {
        cycle_msg += ESS_CABLE_SYMBOL[(power_old >= mw_power_limit) && (mw_power_limit >= MW_MAX_POWER)];
        if (power_grid_to_ess > power_pv_to_ess) cycle_msg += GRID_FLOW_SYMBOL[1 + 2*(power_old >= mw_power_limit) + ((power_old >= mw_power_limit) && (mw_power_limit >= MW_MAX_POWER))];
        else cycle_msg += PV_FLOW_SYMBOL[1 + 2*(power_old >= mw_power_limit) + ((power_old >= mw_power_limit) && (mw_power_limit >= MW_MAX_POWER))];
        cycle_msg += CABLE_SYMBOL[(power_old >= mw_power_limit) && (mw_power_limit >= MW_MAX_POWER)];
    }
    if (int(round(power_ess)) < 0) {
        cycle_msg += ESS_CABLE_SYMBOL[power_old == HM_MAX_POWER];
        cycle_msg += ESS_FLOW_SYMBOL[2 + (power_old == hm_power_limit) + (power_old == HM_MAX_POWER)];
        cycle_msg += CABLE_SYMBOL[power_old == HM_MAX_POWER];
    }
    if (int(round(power_ess)) == 0) {
        cycle_msg += ESS_CABLE_SYMBOL[0];
        if (!mw_power_limit) cycle_msg += PV_FLOW_SYMBOL[2];
        else cycle_msg += ESS_FLOW_SYMBOL[!hm_power_limit];
        cycle_msg += CABLE_SYMBOL[0];
    }
    // Determine vbat at cbat=0 for correct battery level calculation
    int vbat_idle = vbat-round(cbat/16.0);
    if (vbat_idle > BAT_EMPTY) cycle_msg += BAT_LEVEL_SYMBOL[min(((vbat_idle-BAT_EMPTY)*(BAT_LEVELS-2))/(BAT_FULL-BAT_EMPTY)+1,BAT_LEVELS-1)];
    else cycle_msg += BAT_LEVEL_SYMBOL[0];
    ...
    // Grid power
    sprintf(buf,"%4d",int(round(power_grid)));
    cycle_msg += buf;
    cycle_msg += GRID_SYMBOL;
    if (int(round(power_grid)) >= 0) cycle_msg += GRID_FLOW_SYMBOL[int(round(power_grid)) > 0];
    else {
        if (power_ess_to_grid > power_pv_to_grid) cycle_msg += ESS_FLOW_SYMBOL[2];
        else cycle_msg += PV_FLOW_SYMBOL[5];
    }
    cycle_msg += GRID_CABLE_SYMBOL;

    // Power consumption
    cycle_msg += CONS_CABLE_SYMBOL;
    if (power_grid_to_cons > max(power_pv_to_cons, power_ess_to_cons)) cycle_msg += GRID_FLOW_SYMBOL[1];
    else {
        if (power_ess_to_cons > power_pv_to_cons) cycle_msg += ESS_FLOW_SYMBOL[5];
    ...
    if ((millis()-ts_HM)/1000 >= HOYMILES_KEEPALIVE) {
        HoymilesCommand(power_new < 0);
        return;
    }

    // Keep alive Meanwell relay (if Meanwell is turned on)
    if (mw_on && ((millis()-ts_MW)/1000 >= MW_TIMER-30)) {
    ...
    switch (command) {
        ...
        case 'n':
            ...
            if (en_from_pv) {
                ...
                cmd_resp += buf;
                cmd_resp += SUN_SYMBOL[0];
                cmd_resp += PV_FLOW_SYMBOL[1];
                cmd_resp += PV_CABLE_SYMBOL[0];
                cmd_resp += ESS_CABLE_SYMBOL[0];
                cmd_resp += PV_FLOW_SYMBOL[1];
                cmd_resp += ESS_SYMBOL;
            ...
            cmd_resp += "\r\nPV loss to ESS: ";
            cmd_resp += String(en_pv_wasted/1000,3);
            cmd_resp += "/";
            cmd_resp += String(en_from_pv/1000,3);
            if (en_pv_wasted < en_from_pv) {
                sprintf(buf," (%.1f %%)",en_pv_wasted/en_from_pv*100);
                cmd_resp += buf;
            }
            cmd_resp += "\r\n\n";
            break;
...
void SetMWPower(int power) {
    ...
    float current = float(power)/vbat;
