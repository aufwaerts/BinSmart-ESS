...
void loop() {
    ...
    UserCommand(false);  // Repeat command response from previous cycle
    ...
    while (millis()-ts_power < max(PROCESSING_DELAY, uvp_sleep_mode * UVP_SLEEP_DELAY))
        if (UserCommand(true)) break;  // max one command per cycle
...
void CheckErrors() {
    ...
    while (true) {
        ...
        FlashLED(false);
        ts_power = millis();
        while (millis()-ts_power < PROCESSING_DELAY) UserCommand(true);
    }
}
